...
// amounts 1 .. 10000
#define NUM_MULTIPLES_1ERG 10000

// amounts 50 .. 21000000
#define NUM_MULTIPLES_50ERG 420000
...
BOOST_AUTO_TEST_CASE(compress_amounts) {
    ...
    BOOST_CHECK(TestPair(FIXOSHI, 0x1));
    ...
    for (int64_t i = 1; i <= NUM_MULTIPLES_UNIT; i++) {
        BOOST_CHECK(TestEncode(i * FIXOSHI));
    }
    ...
    for (int64_t i = 1; i <= NUM_MULTIPLES_1ERG; i++) {
        BOOST_CHECK(TestEncode(i * COIN));
    }

    for (int64_t i = 1; i <= NUM_MULTIPLES_50ERG; i++) {
