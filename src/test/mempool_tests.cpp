...
BOOST_AUTO_TEST_CASE(TestPackageAccounting) {
    ...
    for (size_t i = 0; i < maxOutputs; i++) {
        parentOfAll.vout.emplace_back(10 * FIXOSHI, CScript() << OP_TRUE);
    }
    ...
    for (size_t totalTransactions = 0; totalTransactions < 100;
        ...
        for (size_t output = InsecureRandRange(maxOutputs) + 1; output > 0;
             output--) {
            tx.vout.emplace_back(10 * FIXOSHI, CScript() << OP_TRUE);
        }
        ...
        Amount randFee = int64_t(InsecureRandRange(300)) * FIXOSHI;
...
BOOST_AUTO_TEST_CASE(MempoolRemoveTest) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        txParent.vout[i].nValue = 33000 * FIXOSHI;
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        txChild[i].vout[0].nValue = 11000 * FIXOSHI;
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        txGrandChild[i].vout[0].nValue = 11000 * FIXOSHI;
    }
...
BOOST_AUTO_TEST_CASE(MempoolIndexingTest) {
    ...
    pool.addUnchecked(entry.Fee(10000 * FIXOSHI).FromTx(tx1));
    ...
    pool.addUnchecked(entry.Fee(20000 * FIXOSHI).FromTx(tx2));
    ...
    pool.addUnchecked(entry.Fee(15000 * FIXOSHI).FromTx(tx4));
    ...
    entry.nTime = 1;
    pool.addUnchecked(entry.Fee(10000 * FIXOSHI).FromTx(tx5));
    ...
    std::string dummy;
    BOOST_CHECK_EQUAL(
        pool.CalculateMemPoolAncestors(entry.Fee(2000000 * FIXOSHI).FromTx(tx7),
    ...
    BOOST_CHECK_EQUAL(pool.CalculateMemPoolAncestors(
                          entry.Fee(200000 * FIXOSHI).Time(4).FromTx(tx10),
...
BOOST_AUTO_TEST_CASE(MempoolAncestorIndexingTest) {
    ...
    Amount fee = int64_t((20000 / tx2Size) * (tx7Size + tx6Size) - 1) * FIXOSHI;
    ...
    pool.addUnchecked(entry.Fee(Amount(5000 * FIXOSHI)).FromTx(tx8));
...
BOOST_AUTO_TEST_CASE(MempoolSizeLimitTest) {
    ...
    pool.addUnchecked(entry.Fee(5000 * FIXOSHI).FromTx(tx2));
    ...
    pool.addUnchecked(entry.Fee(20000 * FIXOSHI).FromTx(tx3));
    ...
    CFeeRate maxFeeRateRemoved(25000 * FIXOSHI,
    ...
    pool.addUnchecked(entry.Fee(7000 * FIXOSHI).FromTx(tx4));
    pool.addUnchecked(entry.Fee(1000 * FIXOSHI).FromTx(tx5));
    pool.addUnchecked(entry.Fee(1100 * FIXOSHI).FromTx(tx6));
    pool.addUnchecked(entry.Fee(9000 * FIXOSHI).FromTx(tx7));
    ...
    BOOST_CHECK_EQUAL(
        ...
        (maxFeeRateRemoved.GetFeePerK() + feeIncrement) / 8 + FIXOSHI);
...
BOOST_AUTO_TEST_CASE(TestImportMempool) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        chainedTxn[i].vout.emplace_back(10 * FIXOSHI, CScript() << OP_TRUE);
...
BOOST_AUTO_TEST_CASE(MempoolAncestryTests) {
    ...
    pool.addUnchecked(entry.Fee(10000 * FIXOSHI).FromTx(tx2));
    ...
    CTransactionRef tx3 = make_tx(/* output_values */ {290 * CENT, 200 * CENT},
                                  /* inputs */ {tx2});
    pool.addUnchecked(entry.Fee(10000 * FIXOSHI).FromTx(tx3));
    ...
    pool.addUnchecked(entry.Fee(10000 * FIXOSHI).FromTx(tx4));
    ...
    for (uint64_t i = 0; i < 5; i++) {
        ...
        v -= 50 * CENT;
        pool.addUnchecked(entry.Fee(10000 * FIXOSHI).FromTx(tyi));
    ...
    pool.addUnchecked(entry.Fee(10000 * FIXOSHI).FromTx(ty6));
    ...
    td = make_tx(/* output_values */ {6 * COIN}, /* inputs */ {tb, tc},
                 /* input_indices */ {0, 0});
    pool.clear();
    pool.addUnchecked(entry.Fee(10000 * FIXOSHI).FromTx(ta));
    pool.addUnchecked(entry.Fee(10000 * FIXOSHI).FromTx(tb));
    pool.addUnchecked(entry.Fee(10000 * FIXOSHI).FromTx(tc));
    pool.addUnchecked(entry.Fee(10000 * FIXOSHI).FromTx(td));
